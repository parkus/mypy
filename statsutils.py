...
def multivariate_normal_loglike(x, mean=None, cov=None):
    p, n = x.shape
    if mean is None:
    ...
    # ummm time for some gnarly vectorizing to do the matrix sum for each
    # term. this matrix sum is (x_i - mean).T * cov**-1 * (x_i - mean)
    # where the matrices in the product have shapes 1 x n, n x n, n x 1
    xx = x - mean[:,None] # p x n
    element_prod = x[:,None,:]*inv_cov[:,:,None] # p x p x n
    matrix_prod_first = np.sum(element_prod, 0) # p x n
    element_prod = matrix_prod_first * xx # p x n
    terms = matrix_prod_final = np.sum(element_prod, 0)
    result = -n*p/2*np.log(2*np.pi) - n/2*np.log(np.linalg.det(cov)) \
             - 0.5*np.sum(terms)
    return result


def draw_arbitrary(n, icdf):
    """
    Draw random variables from an arbitrary 1D distribution given the
    inverse cdf. The heavy lifting is on the user to determine the inverse
    cdf function.
    ...
    n : integer
    icdf : function
        When given a cumulative probability on [0,1], should return the value
        corresponding to that probability (i.e. gives the x value for a plot
        where y is the cdf).

    Returns
    -------
    values of n random draws
    """
